...
router.post('/', async (req, res) => {
    ...
    if (error)
        return res.status(400).send({
            error: { message: 'موبایل ارسالی دارای فرمت اشتباه است' },
        });
    ...
    if (!user)
        return res.status(400).send({
            error: {
                message: 'شماره شما در سیستم وجود ندارد. ابتدا ثبت نام کنید.',
            },
        });
    if (user.isPhoneNumberValidated)
        return res
            .status(400)
            .send({ error: { message: 'موبایل شما قبلا تایید شده است.' } });
...
router.post('/validate', async (req, res) => {
    ...
    if (error)
        return res
            .status(400)
            .send({ error: { message: 'کد ارسالی دارای فرمت اشتباه است' } });
    ...
    if (result.validated) {
        ...
        return res
            .status(400)
            .send({ status: 0, error: { message: 'کد ارسالی اشتباه است' } });
