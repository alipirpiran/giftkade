...
router.post('/', async (req, res) => {
    ...
    if (!user)
        return res
            .status(400)
            .send({ error: { message: 'شما ثبت نام نکرده اید' } });
...
router.post('/validate', async (req, res) => {
    ...
    if (error)
        return res.status(400).send({
            error: { message: 'لطفا ورودی هارا کنترل کنید', dev: error },
        });
    ...
    if (!user)
        return res.status(400).send({
            error: { message: 'کاربری با این شماره موبایل یافت نشد' },
        });
...
router.post('/change/:token', async (req, res) => {
    ...
    if (!token)
        return res
            .status(400)
            .send({ error: { message: 'بازنشانی مورد نظر یافت نشد' } });

    if (token.isUsed)
        return res
            .status(400)
            .send({ error: { message: 'رمزعبور تغیر کرده است' } });

    let difSecs = (Date.now() - token.date) / 1000;
    if (difSecs > limitSecs)
        return res.status(400).send({
            error: {
                message: 'زمان اعتبار بازنشانی رمزعبور به اتمام رسیده است',
            },
        });
    ...
    return res
        .status(200)
        .send({ status: 1, message: 'رمزعبور با موفقیت تغییر کرد' });
});
